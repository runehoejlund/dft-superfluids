...
def make_figure(figname, xmax, N, postfix=''):
    qeh_data = np.load('qeh_data' + '_nFilling=1_nPadding=0.npz')
    qeh_data_E_b = qeh_data['E_b']
    qeh_data_bilayers = list(qeh_data['bilayers'])
    # qeh_data_bilayers = ["_".join(b.split(", ")) for b in qeh_data['bilayers']]
    vmin = np.min(qeh_data_E_b)*10**3
    vmax = np.max(qeh_data_E_b)*10**3
    ...
    for i, filename in enumerate(filenames):
        ...
        try:
            _E_b.append(qeh_data_E_b[qeh_data_bilayers.index(bilayer)])
        except:
    ...
    for i, name in enumerate(names_sorted):
        ...
        cutoff = round(dens_sorted[i]*10000)
    ...
    ax2.set_xlim([0, xmax])
    plt.text(xmax - 8*xmax/41, 0.87, 'BEC', fontsize=12)
    plt.text(xmax - 8*xmax/41, 0.07, 'BCS', fontsize=12)
    ax2.grid()
    fig2.tight_layout()
