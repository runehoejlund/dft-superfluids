...
#load
out=np.load('./01_materials_stable.npz')
...
plt.yticks(fontsize=fS)
plt.tight_layout()
plt.savefig('./plots/thermo_stab.png')
plt.close()
#convew hull
plt.figure(figsize=(9, 5))
...
with open('02a_uid_for_which_we_need_a_thickness.txt', "w") as output:
...
with open('02a_thicknesses.txt', 'r') as content:
    for line in content:
        #x=line[:-1]
...
np.savez('./02_materials_iso_stable.npz',uid_vec_eiso=uid_vec_eiso,uid_vec_hiso=uid_vec_hiso,uid_vec_iso=uid_vec_iso,Mat_plot_iso=Material_plot[iso],Material_plot=Material_plot,materials=materials,materials_e_iso=materials_e_iso,materials_h_iso=materials_h_iso,h_iso=h_iso,e_iso=e_iso,e_avg_vec=e_avg_vec,h_avg_vec=h_avg_vec,bandgap_iso=bandgap_iso,dir_bandgap_iso=dir_bandgap_iso,d_List=d_List)