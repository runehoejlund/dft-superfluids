...
def get_E_b_for_hs(e_layer, h_layer, nFilling, nPadding):
    '''
    @returns: E_b
    '''
    Hartree = ase.units.Hartree
    ...
    ee, ev = get_exciton_binding_energies_coulomb(eff_mass=get_intermass('H-' + e_layer, 'H-' + h_layer),
                                            e_distr=e_distr,
                                            h_distr=h_distr)
...
def calculate_and_save_binding_energies(materials, nPadding, nFilling):
    ...
    for (i_e, e_layer) in enumerate(materials):
        for (i_h, h_layer) in enumerate(materials):
            ...
            E_b[count] = get_E_b_for_hs(e_layer, h_layer, nFilling, nPadding)
