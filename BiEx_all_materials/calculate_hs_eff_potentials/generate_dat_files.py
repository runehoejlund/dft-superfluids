...
def calculate_and_save_dat(materials_e,materials_h,e_avg_vec_iso,h_avg_vec_iso,Mat_plot_iso,d_List,nPadding, nFilling):
    def get_thickness(layer):
        if layer == 'BN':
            ...
            return d_List[layer==Mat_plot_iso][0]
    ...
    for (i_e, e_layer) in enumerate(materials_e):
        for (i_h, h_layer) in enumerate(materials_h):
            ...
            print(layers)
            thicknesses = [get_thickness(layer) for layer in layers]
            ...
            print(np.shape(U_eh))
            print(np.shape(U_ee))
            with open('../out/n-' + e_layer + '_' + 'p-' + h_layer+'.dat', 'w') as dat_file:
                dat_file.write(str(rows))
                for i in range(rows):
