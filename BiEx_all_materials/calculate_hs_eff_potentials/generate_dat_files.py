...
def calculate_and_save_dat(nPadding=0, nFilling=1):
    vdWH = np.load('vdWH_nFilling=' + str(nFilling) + '_nPadding=' + str(nPadding) + '.npz')
    bilayers = [b.replace(', ', '_') for b in vdWH['bilayers']]
    q = vdWH['q']
    q_au = q * Bohr
    N_q=len(q_au)

    print([k for k in vdWH.keys()])
    ...
    for i, bilayer in enumerate(bilayers):
        ...
        with open('../out/' + bilayer + '_QEH.dat', 'w') as dat_file:
            ...
            U_eh = np.real(vdWH['U_eh'][i].flatten())
            U_ee = np.real(vdWH['U_ee'][i].flatten())
            U_hh = np.real(vdWH['U_hh'][i].flatten())
        ...
        with open('../out/' + bilayer + '_analytic.dat', 'w') as dat_file:
            dat_file.write(str(N_q))
            U_eh = vdWH['U_eh_analytic'][i]
            U_ee = vdWH['U_ee_analytic'][i]
            U_hh = vdWH['U_hh_analytic'][i]
